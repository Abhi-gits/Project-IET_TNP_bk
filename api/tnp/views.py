...
def PlacementDetail(request, pk):
    try:
        ...
        return Response(
            {"message": "Placement does not exist"}, status=status.HTTP_404_NOT_FOUND
        )
...
def PlacementDelete(request, pk):
    ...
    return Response(
        {"message": "Placement deleted successfully"}, status=status.HTTP_204_NO_CONTENT
    )
...
def CoursesDetail(request, pk):
    try:
        ...
        return Response(
            {"message": "Courses does not exist"}, status=status.HTTP_404_NOT_FOUND
        )
...
def CoursesDelete(request, pk):
    ...
    courses_item.delete()
    return Response(
        {"message": "Courses deleted successfully"}, status=status.HTTP_204_NO_CONTENT
    )
